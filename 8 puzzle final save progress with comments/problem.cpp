...
bool problem::goal_test(int data[]){
	...
	for(int i = 0; i < PROBLEM_SIZE - 1; i++){  //go through the puzzle of whatever size to check if it is solved
		//cout << data[i] << " " << i + 1 << endl; used to test functionality of goal_test
		...
		for(int i = 0; i < PROBLEM_SIZE; i++){
			...
			if(state[i] != (i + 1)){ //current tile is misplaced, now need to calculate how many moves to correct position
				...
				if(state[i] != 0){
					...
					//int difference = std::abs(state[i] - (i+1));
				
					//cout << "misplaced tile: " << state[i] <<endl;
					//int moves_to_destination = (difference % 3) + (difference / 3); //if 4x4 puzzle, change 3 to 4
					//heuristic += moves_to_destination;
...
void problem::expand(std::queue<problem>& nodes, problem p, int algorithm){
	...
	int children = 0;
	//int down_star, up_star, left_star, right_star; 
	...
	if(can_down == true){
		children++;
		//problem d = problem(state, PROBLEM_SIZE);
		problem down_child = problem(p.state, p , 1, algorithm);
		down_child.assign_heuristic(algorithm);
		//down_star = down_child.heuristic + down_child.cost;
		nodes.push(down_child);
		//cout << "Printing child from slide down: " << endl;
		down_child.print_state();
	}
	if(can_up == true){
		children++;
		problem up_child = problem(p.state, p , 2, algorithm);
		
		up_child.assign_heuristic(algorithm);
		//up_star = up_child.heuristic + up_child.cost;
		nodes.push(up_child);
		//cout << "Printing child from slide up: " << endl;
		up_child.print_state();
	}
	if(can_left == true){
		children++;
		problem left_child = problem(p.state, p , 3, algorithm);
		left_child.assign_heuristic(algorithm);
		//left_star = left_child.heuristic + left_child.cost;
		nodes.push(left_child);
		//cout << "Printing child from slide left: " << endl;
		left_child.print_state();
	}
	if(can_right == true){
		children++;
		problem right_child = problem(p.state, p , 4, algorithm);
		right_child.assign_heuristic(algorithm);
		//right_star = right_child.heuristic + right_child.cost;
		nodes.push(right_child);
		//cout << "Printing child from slide right: " << endl;
		right_child.print_state();
	}
	
	//cout << "Number of children: " << children << endl;
	
	//for(int i = 0; i < children; i++){  //TODO: FIND WAY TO QUEUE ELEMENTS IN ORDER OF COST
